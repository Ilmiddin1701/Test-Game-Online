...
class Test50Fragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.apply {
            ...
            var completedTest50 = 0
            var completedTestTime50 = 0
            ...
            btnNext.setOnClickListener {
                if (isChecked) {
                    ...
                    if (completedTest == 50) {
                        ...
                        if (updateTime.toString().toInt() < 210000 && answerTrue == 50) {
                            reference.child(MySharedPreferences.userId).child("tests")
                                    ...
                                    .addValueEventListener(object : ValueEventListener {
                                        override fun onDataChange(snapshot: DataSnapshot) {
                                            ...
                                            snapshot.children.forEach { child ->
                                                ...
                                                if (value != null && value.tests == 50) {
                                                    completedTest50 += value.testCompleted!!
                                                    completedTestTime50 += value.testCompletedTime!!.toInt()
                                                }
                                            }
                                            reference.child(MySharedPreferences.userId)
                                                .child("allTestsCompleted50").setValue(completedTest50)
                                            reference.child(MySharedPreferences.userId)
                                                .child("allTestsCompletedTime50").setValue(completedTestTime50)
                                        }
                                        override fun onCancelled(error: DatabaseError) {
                                            Toast.makeText(context, "Error!", Toast.LENGTH_SHORT).show()
                                        }
                            ...
                            dialog.show()
                        } else if (updateTime in 210000..240000 && answerTrue == 50) {
                            reference.child(MySharedPreferences.userId).child("tests")
                            ...
                            dialog.setView(dialogItem.root)
                            dialog.show()
                        } else if (updateTime in 240000..270000 && answerTrue == 50) {
                            reference.child(MySharedPreferences.userId).child("tests")
