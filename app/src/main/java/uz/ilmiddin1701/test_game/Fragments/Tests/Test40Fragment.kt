...
class Test40Fragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.apply {
            ...
            var completedTest40 = 0
            var completedTestTime40 = 0
            ...
            btnNext.setOnClickListener {
                if (isChecked) {
                    ...
                    if (completedTest == 40) {
                        ...
                        if (updateTime.toString().toInt() < 180000 && answerTrue == 40) {
                            reference.child(MySharedPreferences.userId).child("tests")
                                ...
                                reference.child(MySharedPreferences.userId).child("tests")
                                    .addValueEventListener(object : ValueEventListener {
                                        override fun onDataChange(snapshot: DataSnapshot) {
                                            ...
                                            snapshot.children.forEach { child ->
                                                val value =
                                                    child.getValue(TestLevelData::class.java)
                                                if (value != null) {
                                                    completedTest40 += value.testCompleted!!
                                                    completedTestTime40 += value.testCompletedTime!!.toInt()
                                                }
                                            }
                                            reference.child(MySharedPreferences.userId)
                                                .child("allTestsCompleted40").setValue(completedTest40)
                                            reference.child(MySharedPreferences.userId)
                                                .child("allTestsCompletedTime40").setValue(completedTestTime40)
                                        }
                                        override fun onCancelled(error: DatabaseError) {
                                            Toast.makeText(context, "Error!", Toast.LENGTH_SHORT).show()
                                        }
                            ...
                            dialog.show()
                        } else if (updateTime in 180000..210000 && answerTrue == 40) {
                            reference.child(MySharedPreferences.userId).child("tests")
                            ...
                            dialog.setView(dialogItem.root)
                            dialog.show()
                        } else if (updateTime in 210000..240000 && answerTrue == 40) {
                            reference.child(MySharedPreferences.userId).child("tests")
