...
class Test20Fragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.apply {
            ...
            var completedTest20 = 0
            var completedTestTime20 = 0
            ...
            btnNext.setOnClickListener {
                if (isChecked) {
                    ...
                    if (completedTest == 20) {
                        ...
                        if (updateTime.toString().toInt() < 90000 && answerTrue == 20) {
                            reference.child(MySharedPreferences.userId).child("tests")
                                    ...
                                    .addValueEventListener(object : ValueEventListener {
                                        override fun onDataChange(snapshot: DataSnapshot) {
                                            ...
                                            snapshot.children.forEach { child ->
                                                val value = child.getValue(TestLevelData::class.java)
                                                if (value != null && value.tests == 20) {
                                                    completedTest20 += value.testCompleted!!
                                                    completedTestTime20 += value.testCompletedTime!!.toInt()
                                                }
                                            }
                                            reference.child(MySharedPreferences.userId)
                                                .child("allTestsCompleted20").setValue(completedTest20)
                                            reference.child(MySharedPreferences.userId)
                                                .child("allTestsCompletedTime20").setValue(completedTestTime20)
                                        }
                                        override fun onCancelled(error: DatabaseError) {
                                            Toast.makeText(context, "Error!", Toast.LENGTH_SHORT).show()
                                        }
                        ...
                        } else if (updateTime.toString()
                                .toInt() in 90000..120000 && answerTrue == 20
                        ) {
                            reference.child(MySharedPreferences.userId).child("tests")
                            ...
                            dialog.show()
                                ...
                                .toInt() in 120000..150000 && answerTrue == 20
                        ) {
                            reference.child(MySharedPreferences.userId).child("tests")
